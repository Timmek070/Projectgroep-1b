...
class RegisterTestConditionCoverage {
    ...
    void registerUser2() {
        ...
        for (User user : users) {
            if (Database.getInstance().getUsers().get(4).getUsername().equals(user.getUsername())) {
                assertEquals(Database.getInstance().getUsers().get(4).getUsername(), user.getUsername());
            }
        }
    }
}